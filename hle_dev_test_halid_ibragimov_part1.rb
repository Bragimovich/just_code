...
result.each do |row|
   ...
   if row['candidate_office_name'].to_s.count('/') > 1
     string_two_slash = row['candidate_office_name'].to_s
     string_two_slash.to_s.strip.gsub('//', '/')
     string_for_first_condition = string_two_slash.split('/')
     ...
     arr_for_string = []
        string_for_first_condition.each_with_index do |elem, idx|
         ...
         first_condition.gsub!('County county', 'County')
         first_condition.gsub!('McLean County board member and', 'McLean County board member')
         first_condition
             query = <<~SQL
